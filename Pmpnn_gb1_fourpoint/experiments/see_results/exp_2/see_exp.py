...
def load_results(results_dir):
    ...
    for fname in os.listdir(results_dir):
        ...
        assays_above_threshold_not_in_subset = res.get("assays_above_threshold_not_in_subset", [])
        num_assayed = res.get("num_assayed", 0)
        num_in_train = res.get("num_in_train", 0)
        denominator = num_assayed - num_in_train
        if denominator > 0 and assays_above_threshold_not_in_subset is not None:
            entry["proportion_assays_above_threshold_not_in_subset"] = len(assays_above_threshold_not_in_subset) / denominator
        else:
            entry["proportion_assays_above_threshold_not_in_subset"] = np.nan
...
def average_across_seeds(entries):
    ...
    metric_keys = [
        ...
        "proportion_assays_above_threshold_not_in_subset",
    ]
...
def build_grids(entries, averaged_entries, metric):
    """
    Build grids of shape (len(learning_rates), len(random_seeds) + 1) for each loss_type.
    The last column is the average across seeds.

    Returns:
        loss_types: sorted list of unique loss_type values
        random_seeds: sorted list of unique random_seed values
        learning_rates: sorted list of unique learning_rate values
        grids: dict mapping loss_type -> 2D numpy array [len(learning_rates), len(random_seeds) + 1]
    """
    random_seeds = sorted({e["random_seed"] for e in entries if "random_seed" in e})
    learning_rates = sorted({e["learning_rate"] for e in entries})
    loss_types = sorted({e["loss_type"] for e in entries})

    seed_index = {seed: i for i, seed in enumerate(random_seeds)}
    lr_index = {lr: i for i, lr in enumerate(learning_rates)}

    grids = {}
    for lt in loss_types:
        grid = np.full((len(learning_rates), len(random_seeds) + 1), np.nan, dtype=float)
        grids[lt] = grid

    for e in entries:
        if "random_seed" not in e:
            continue
        seed = e["random_seed"]
        lr = e["learning_rate"]
        lt = e["loss_type"]
        if lt not in grids or seed not in seed_index:
            continue
        i_lr = lr_index[lr]
        i_seed = seed_index[seed]
        value = e.get(metric, np.nan)
        grids[lt][i_lr, i_seed] = value

    averaged_dict = {}
    for e in averaged_entries:
        key = (e["learning_rate"], e["loss_type"])
        averaged_dict[key] = e.get(metric, np.nan)
    ...
    for lt in loss_types:
        for lr in learning_rates:
            ...
            if key in averaged_dict:
                i_lr = lr_index[lr]
                grids[lt][i_lr, len(random_seeds)] = averaged_dict[key]

    return loss_types, random_seeds, learning_rates, grids


def plot_metric_heatmaps(entries, averaged_entries, metric, out_dir):
    """
    For a given metric, create a figure with subplots for each loss_type.
    X-axis: random_seed (individual seeds + average)
    Y-axis: learning_rate
    """
    loss_types, random_seeds, learning_rates, grids = build_grids(entries, averaged_entries, metric)
    ...
    all_grids = [grids[lt] for lt in loss_types]
    vmin = np.nanmin(all_grids)
    vmax = np.nanmax(all_grids)

    x_labels = [f"seed_{seed}" for seed in random_seeds] + ["average"]
    ...
    for j_lt, lt in enumerate(loss_types):
        ...
        ax.set_xlabel("random_seed")

        ax.set_xticks(np.arange(len(random_seeds) + 1))
        ...
        for iy in range(len(learning_rates)):
            for ix in range(len(random_seeds) + 1):
                val = grid[iy, ix]
                if np.isnan(val):
                    continue
...
def main():
    ...
    results_dir = os.path.abspath(os.path.join(base_dir, "../../results/exp_2/"))
    ...
    for metric in metrics:
        plot_metric_heatmaps(entries, averaged_entries, metric, out_dir)
