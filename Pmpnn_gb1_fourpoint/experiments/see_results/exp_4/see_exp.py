...
def build_grids(entries, metric):
    """
    Build grids of shape (len(learning_rates), len(alphas)) for each loss_type.
    ...
    Returns:
        ...
        grids: dict mapping loss_type -> 2D numpy array [len(learning_rates), len(alphas)]
    """
    ...
    for e in entries:
        ...
        grids[lt][i_lr, i_a] = value

    return loss_types, alphas, learning_rates, grids
...
def plot_metric_heatmaps(entries, metric, out_dir):
    ...
    """
    loss_types, alphas, learning_rates, grids = build_grids(entries, metric)
...
def main():
    ...
    results_dir = os.path.abspath(os.path.join(base_dir, "../../results/exp_4/"))
